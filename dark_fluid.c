...
int test1=0;
double rho_0_c, m, V0, omega_rad, omega_b, omega_phi, domega_dx, omega0_phi, omega0_b, omega0_rad, H0,omega_rad_adapt, omega_b_adapt, omega_phi_adapt, Q, lambda;
...
double calcul(double y[], double x1, double x2, int nvar, void (*derivs)(double, double [], double []))
{
  ...
  for (nstp=1;nstp<=MAXSTP;nstp++) { 
	...
	atemp=y[1];
	rho_phi= ( 0.5 * y[3]*y[3] + 0.5*Q*Q/y[2]/y[2]/pow(y[1],6.) + V_pot(y[2]));
	omega_rad = omega0_rad * pow(y[1],-4.);
	omega_b = omega0_b * pow(y[1],-3.);
	omega_phi= omega0_phi *rho_phi;
	output=fopen(SORTIE,"a");
	fprintf(output,"%.10e %.10e %.10e %.10e %.10e %.10e %.10e %.10e\n", x, y[1], y[2], y[3], omega_rad, omega_b, omega_phi, omega_b/omega_phi);
	fclose(output);
      }
    }
    for (i=1;i<=nvar;i++)
      yscal[i]=fabs(y[i])+fabs(dydx[i]*h)+TINY;
    if (difftime(time(0),T1)>4)
      { omega0_phi=( 0.5 * y[3]*y[3] + 0.5*Q*Q/y[2]/y[2]/pow(y[1],6.) + V_pot(y[2]))*omega0_phi*pow(y[1],3.);
	while(y[1]<1) {
		y[1]=y[1]*pow(10.,0.05);
		omega_rad = omega0_rad * pow(y[1],-4.);
		omega_b = omega0_b * pow(y[1],-3.);
		omega_phi= omega0_phi * pow(y[1],-3.) + V0/rho_0_c;
		output=fopen(SORTIE,"a");
		fprintf(output,"%.10e %.10e %.10e %.10e %.10e %.10e %.10e %.10e\n", x, y[1], y[2], y[3], omega_rad, omega_b, omega_phi, omega_b/omega_phi);
		fclose(output);
	}
...

{
  ...
  FILE *ext;
  	...
  	val[2]= 1.8479146404e+04;  	   /* valeur initiale de sigma */
