...
int test1=0;
double rho_0_c, m, omega_rad, omega_b, omega_phi, domega_dx, omega0_phi, omega0_b, omega0_rad, H0,omega_rad_adapt, omega_b_adapt, omega_phi_adapt, Q,omega_de, lambda, alpha;

/*---------------------------------------------------------------------------------------------------------------*/

double V_pot(double x)
{
	return 0.5*m*m*x*x+0.25*lambda*pow(x,4)+1./6.*alpha*pow(x,6);
}


double dV_pot(double x)
{
	return m*m*x+lambda*pow(x,3)+alpha*pow(x,5);
}
...

{
  ...
  FILE *ext;
  	...
  	val[2]= 1.e1;  	   /* valeur initiale de sigma */
  	val[3]= 1.e1; 		   /* valeur initiale de dsigma/dt */
  m=1.e-24 * e/h_;      /* voir article 2 */
  Q=0.*6.96e-18;              /* tuning pour trouver omega_b/omega_phi = 0.2 */ 
  lambda= 1.e-100*pow(c,5)/h_;
  alpha=1.e-156*pow(c,10)/(pow(m*h_,2));
