...
int test1=0;
double rho_0_c, m, omega_rad, omega_b, omega_phi, domega_dx, omega0_phi, omega0_b, omega0_rad, H0,omega_rad_adapt, omega_b_adapt, omega_phi_adapt, Q, a_eq_exp, cut, alpha, beta, omega_de;

/*---------------------------------------------------------------------------------------------------------------*/
double V_pot(double x)
{
	return alpha*(exp(beta*x*x)-1);
}


double dV_pot(double x)
{
	return 2*alpha*beta*x*exp(beta*x*x);
}
...
double f(double CI, double val[], double x1, int nvar, void (*derivs)(double, double [], double []))
{
  ...
  for (nstp=1;nstp<=MAXSTP;nstp++) { 
    ...
    if (difftime(time(0),T1)>5)
      { a_eq=omega0_rad/(omega0_b+omega0_phi*(0.5*y[3]*y[3]+V_pot(y[2]))*pow(y[1],3.));
...

{
  ...
  Q=0*3.96e-24;              /* tuning pour trouver omega_b/omega_phi = 0.2 */
  ...
  double ax=3.34e-11,bx=4.6e-11,cx=8.e-11,xmin=0.;
