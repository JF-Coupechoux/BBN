...
int test1=0;
double rho_0_c, m, V0, omega_rad, omega_b, omega_phi, domega_dx, omega0_phi, omega0_b, omega0_rad, H0,omega_rad_adapt, omega_b_adapt, omega_phi_adapt, Q, lambda1, alpha1, lambda2, alpha2,H0;

/*---------------------------------------------------------------------------------------------------------------*/

double V_pot(double x)
{
	return alpha1*exp(-lambda1*x)+alpha2*exp(-lambda2*x);
}


double dV_pot(double x)
{
	return -lambda1*alpha1*exp(-lambda1*x) -lambda2*alpha2*exp(-lambda2*x);
}
...
{ 
	...
	* pow(val[1],-3)+ 1./(3*H0*H0)*( 0.5 * val[3]*val[3] + V));
...
double calcul(double y[], double x1, double x2, int nvar, void (*derivs)(double, double [], double []))
{
  ...
  for (nstp=1;nstp<=MAXSTP;nstp++) { 
	...
	omega_phi= 1./(3.*H0*H0)*rho_phi;
	output=fopen(SORTIE,"a");
	fprintf(output,"%.10e %.10e %.10e %.10e %.10e %.10e %.10e %.10e \n", x, y[1], y[2], y[3], omega_rad, omega_b, omega_phi, (0.5 * y[3]*y[3] - V_pot(y[2]))/(0.5 * y[3]*y[3] + V_pot(y[2])) );
	fclose(output);
      }
    }
    for (i=1;i<=nvar;i++)
      yscal[i]=fabs(y[i])+fabs(dydx[i]*h)+TINY;
    if (difftime(time(0),T1)>2 || y[1]>1.)
      { 
...
{
  double h,T0,x0,x1,val [N_VARIABLES+1];
  int test;
  FILE *ext;
  	x0=1.e-25 ;                /* valeur initiale de t */
  	val[1]= 1.e-12; 	   /* valeur initiale du parametre d'expansion a/a0*/  
  	val[2]= 0.;   /* valeur initiale de sigma */
  	val[3]= 0.; 		   /* valeur initiale de dsigma/dt */
  
  h=0.673;         /* Constante de Hubble. *rotterdam/
  ...
  lambda1=8.; 
  alpha1=1.e40*3.*H0*H0*V0;     
  lambda2= 0.; 
  alpha2=3.*H0*H0*V0;   
     

  calcul(val,x0,x1,N_VARIABLES,fonction_diff);



  FILE * f;
  f = popen("gnuplot", "w");
  fprintf(f, "load \"test.gnu\" \n");
  fflush(f);
  sleep(5000);
  pclose(f);
  return 0;
